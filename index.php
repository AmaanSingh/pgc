<!DOCTYPE html>
<html>
<head>
	<title>Programmer's Club</title>
	<meta charset="UTF-8">
	<meta http-equiv="X-UA-Compatible" content="IE=edge">
	<link rel="stylesheet" type="text/css" href="index.css">
	<link rel="stylesheet" href="https://use.typekit.net/njb6zth.css">
	<meta name="viewpoint" content="width=device-width, initial-scale=1">
</head>
<body>
<div id="main">
	<nav>
<div class = "logo">
	<img src = "logo.png">
		<ul>
			<li><a href ="index.php">HOME</a></li>
			<li><a href ="projects.php">PROJECTS</a></li>
			<li><a href ="contact.php">CONTACT</a></li>
			<button class = "navy"><a href ="https://github.com/AmaanSingh"> Contact Us</a></button>
		</ul>
</div>
</nav>
<div class = "title">
		<h1>Hello World</h1><br>
		<p>:Being a very simple program in most programming languages, it is often used to illustrate the basic syntax of a programming language for a working program, and as such is often the very first program people write; a new world being opened for new creators.</p>


</div>
<div class = "Sign">
	<h1> PGC</h1>
</div>
<div id="below">
	<div class = "words">
	<h1> The Team that Works </h1>
	<p> A team full of cooperative minds here to help you with your coding needs. From helping you understand the benefits of coding, to building a simple application. </p>
</div>
	<div class = "communicate">
	<h1>Communicate</h1>
	<p> Acquire help from coding experts who can help you one step at a time. No need for any experience, our coding experts have the assests to help you achieve those particular skills.</p>
	<div class = "languages">
	<h1> Languages</h1>
	<p> The languages we are going to explore in our program, tend to be more flexible and easier to learn. These languages are also more significant in the tech world.</p>
</div>
<div class = tools>
	<h1> Tools</h1>
	<p> The tools that we would be working with would include github, repl.it or any cloud ide, modules, packages, and other frameworks. The two prominent web frameworks we would be using is Django and Flask.</p>
</div>

</div>
</div>
<div class = "middle">
	<img src="san.jpg">
	<h1> Our Team Works to help </h1>
	<div class = "box">
		<div><button class = "box"><a href ="projects.html"> <h1>Projects</h1> The code that we have created in our club. This code was added to the website to help you create these basic projects.</a></button></div>
		<div><button class = "box"><a href = "contact.html"><h1>Contact us</h1> If you need more information on our club, just fill out this form to help us reach you.</a></button></div>
		<div><button class= "box"><a href="https://github.com/AmaanSingh"><h1>My Github</h1> If you feel the need to use the club's projects as reference to create your own projects, my github is linked.</a></button></div>
	</div>
</div>
<div class = "footer">

	<div><li><a href = "https://github.com/AmaanSingh"><img src="glyphicons-social-22-github.png"></li></div>
	<div><li><a href = "contact.php"><img src="glyphicons-social-14-e-mail-envelope.png"></a></li></div>
	<div><li><a href = "disclaimer.php">Disclaimer</a></li></div>
	<div class = "computer"><img src="logo.png"></div>
	<div class = "jshs"><img src = "jshslogo.png"></div>

</div>

</body>
</html>
